...
# ---- ElGamal Signature for Nurse/Doctor ----
...
class HospitalSystem:
    def __init__(self):
        ...
        # Rabin key generation for doctor
        self.doctor_p, self.doctor_q, self.doctor_n = rabin_keygen()

        # ElGamal key generation for nurse and admin (DH parameters: p, g)
        self.p = 23  # This can be much larger in real scenarios
        self.g = 5
        self.nurse_public, self.nurse_private = elgamal_keygen(self.p, self.g)
        self.admin_public, self.admin_private = elgamal_keygen(self.p, self.g)

    # Doctor: Add/Update Records (Rabin Encrypted & ElGamal Signed)
    def doctor_add_record(self, patient_id, record_data):
        ...
        signature = elgamal_sign(record_data, self.p, self.g, self.nurse_private)
    ...
    def admin_verify_signature(self, patient_id):
        ...
        if encrypted_record and signature:
            decrypted_record = rabin_decrypt(encrypted_record, self.doctor_p, self.doctor_q)
            if elgamal_verify(decrypted_record, *signature, self.p, self.g, self.nurse_public):
